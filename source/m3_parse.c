	...
	for (u32 f = 0; f < numFunctions; ++f) {
		...
		if (size) {
			...
			if (i_bytes <= i_end) {
				...
				u32 numLocalBlocks;
				_               (ReadLEB_u32(&numLocalBlocks, &ptr, i_end));
				m3log (parse, "    code size: %-4d", size);
				...
				for (u32 l = 0; l < numLocalBlocks; ++l) {
					...
					numLocals += varCount;
					m3log (parse, "      %2d locals; type: '%s'", varCount, c_waTypes[normalType]);
				}
		...
		if (nameType == 1) {
			...
			for (u32 i = 0; i < numNames; ++i) {
				...
				if (index < io_module->numFunctions) {
					if (not io_module->functions[index].name) {
						io_module->functions[index].name = name;
