...
{
	d_m3Assert(o->stackIndex > 0 || IsStackPolymorphic(o));
...
_  (ReadLEB_u32(& localIndex, & o->wasm, o->wasmEnd));             //  printf("--- set local: %d \n", localSlot);
    ...
    if(localIndex < numArgsAndLocals)
    {
...
_          (Pop(o));
        ...
        m3log (parse, "IN FUNCTION %s",o->function->name);
        m3log (parse, "localIndex >= numArgsAndLocals %d>=%d",localIndex, numArgsAndLocals);
...
_  (ReadLEB_u32(& localIndex, & o->wasm, o->wasmEnd));
        u32 numArgsAndLocals = GetFunctionNumArgsAndLocals(o->function);
        if(localIndex >= numArgsAndLocals){
        m3log(module, "In function %s", o->function->name);
        m3log(module, "localIndex >= numArgsAndLocals %d>=%d",localIndex,numArgsAndLocals);
        _throw("local index out of bounds");
    }
...
_          (EmitOp    (o, op));
        ...
        {
            cstr_t moduleName = GetFunctionImportModuleName(function);
            cstr_t functionName = GetFunctionName(function);
            result = ErrorCompile(m3Err_functionImportMissing, o, "'%s.%s'", moduleName, functionName);
        }
    }
